...
MCMCsummary(m3lv3.mcmc, 
          ...
          func_name = "ecdf50")
       mean     sd 2.5%  50% 97.5% Rhat  n.eff ecdf50
xi[14] 7.54 225.59 0.24 1.46 40.81 1.13 150000   0.05
...
MCMCsummary(m3lv4.mcmc, 
          ...
          func_name = "ecdf50")
#       mean    sd 2.5%  50%  97.5% Rhat  n.eff ecdf50
#xi[9] 26.43 625.3 2.69 7.42 103.43 1.13 150000   0.01
...
#      mean  sd 2.5%  50% 97.5% Rhat n.eff ecdf50
#xi[6] 2.38 2.8 0.54 1.47  9.88    1 15338   0.05


### Posteriors of Z and Y for all 3 models (geological, empirical and statistical analogs)
...
par(mfrow=c(6,4), mar=c(5.1, 5.1, 1, 1))
...
#### M1 GVP statistical analogues VEI 3+ eruptions
Mvol <- 18
...
### Posteriors of Z and Y for model 3 (geological, empirical and statistical analogs)
####### Only Model 3 results
postscript("AnalogM3LongGVPpostdens.eps",paper="special",width=2*5*cos (35.4/180*pi)/0.612,
    height=0.5*5*3,onefile = TRUE, horizontal = FALSE)
par(mfrow=c(3,4), mar=c(5.1, 5.1, 1, 1))
...
#### M3 GVP empirical analogues VEI 3+, finding statistical analogues
Mvol <- 23
...
for (i in 2:Mvol)
denslines(m3.Ymcmc[,i],col=colsN[i],lty=i,lwd=2,show.obs = FALSE)
text(5.3,3.7,"(a)",cex=1.5)

densplot(m3.sYmcmc,xlim=c(0,1.5),cex.axis=1.5,cex.lab=1.5,
    main="",xlab=expression(sigma[Y]),ylab="Density",lwd=2)
text(1.46,3,"(b)",cex=1.5)

densplot(m3.Zmcmc[,1],xlim=c(0,3),ylim=c(0,6),cex.axis=1.5,cex.lab=1.5,
    main="",xlab=expression(Z[i]),ylab="Density",lwd=2)
for (i in 2:Mvol)
denslines(m3.Zmcmc[,i],col=colsN[i],lty=i,lwd=2,show.obs = FALSE)
text(2.8,5.7,"(c)",cex=1.5)
densplot(m3.sZmcmc,cex.axis=1.5,cex.lab=1.5,
    main="",xlab=expression(sigma[Z]),ylab="Density",lwd=2)
text(0.77,5.5,"(d)",cex=1.5)



#### M3 GVP statistical analogues VEI 3+
Mvol <- 18
m3.Ymcmc <- m3gvpstat.mcmc[,c(7,17:24,8:16)]
m3.Zmcmc <- m3gvpstat.mcmc[,c(25,35:42,26:34)]
m3.sYmcmc <- m3gvpstat.mcmc[,4]
...
densplot(m3.sZmcmc,xlim=c(0,0.8),cex.axis=1.5,cex.lab=1.5,
    main="",xlab=expression(sigma[Z]),ylab="Density",lwd=2)
text(0.77,6.6,"(h)",cex=1.5)




#### M3 Long record VEI 3+
Mvol <- 3
#### M3
m3.Ymcmc <- m3lv3.mcmc[,c(7:9)]
m3.Zmcmc <- m3lv3.mcmc[,c(10:12)]
...
for (i in 2:Mvol)
denslines(m3.Ymcmc[,i],col=colsN[i],lty=i,lwd=2,show.obs = FALSE)
text(4.8,1.9,"(i)",cex=1.5)

densplot(m3.sYmcmc,cex.axis=1.5,cex.lab=1.5,
    main="",xlab=expression(sigma[Y]),ylab="Density",lwd=2)
for (i in 1:(ceiling(23/2)/2))
  legend(2,1.1-i*0.13,paste("i=",i,sep=""),bty="n",lty=i,col=colsN[i],cex=1.5,lwd=1.8)
for (i in (ceiling(23/2)/2+1):ceiling(23/2))
 legend(6,1.1-(i-ceiling(23/2)/2)*0.13,paste("i=",i,sep=""),bty="n",lty=i,col=colsN[i],cex=1.5,lwd=1.8)
text(9.6,1.06,"(j)",cex=1.5)

densplot(m3.Zmcmc[,1],xlim=c(0,2),ylim=c(0,4.5),cex.axis=1.5,cex.lab=1.5,
    main="",xlab=expression(Z[i]),ylab="Density",lwd=2)
for (i in 2:Mvol)
denslines(m3.Zmcmc[,i],col=colsN[i],lty=i,lwd=2,show.obs = FALSE)
text(1.9,4.2,"(k)",cex=1.5)

densplot(m3.sZmcmc,cex.axis=1.5,cex.lab=1.5,
    main="",xlab=expression(sigma[Z]),ylab="Density",lwd=2)
for (i in (ceiling(23/2)+1):(ceiling(23/2)*2-ceiling(23/2)/2))
  legend(1,2.7-(i-ceiling(23/2))*0.32,paste("i=",i,sep=""),bty="n",lty=i,col=colsN[i],cex=1.5,lwd=1.8)
for (i in (ceiling(23/2)*2-ceiling(23/2)/2+1):23)
 legend(4,2.7-(i-(ceiling(23/2)*2-ceiling(23/2)/2)+1)*0.32,paste("i=",i,sep=""),bty="n",lty=i,col=colsN[i],cex=1.5,lwd=1.8)
text(6.7,2.5,"(l)",cex=1.5)
...
postscript("TongariroV3V4Analog-Forecast.eps",paper="special",
    width=1.5*5*cos (35.4/180*pi)/0.612,
    height=0.5*5*4,onefile = TRUE, horizontal = FALSE)
par(mfrow=c(4,3),mar=c(4.5,4.5,2.2,1))

#################################
## Forecast x[6] from statistical analogues VEI 3+
...
box()
text(4,0.47,"(a)",cex=1.5)
text(3.4,0.34,"M1",cex=1.5)
text(3.4,0.26,"VEI 3+",cex=1.5)
...
axis(1,at=quantile(log(xx[,1,drop=TRUE]),c(0.5))+0.1,
     floor(quantile((xx[,1,drop=TRUE]),c(0.5))*1000+1500),
     line=-0,cex.axis=1.5,lwd=0,lwd.ticks=0)
axis(3,at=quantile(log(xx[,1,drop=TRUE]),c(0.25,0.75)),
     floor(quantile((xx[,1,drop=TRUE]),c(0.25,0.75))*1000+1500),
     line=-0.2,cex.axis=1.5,lwd=0,lwd.ticks=1.5)
axis(3,at=quantile(log(xx[,1,drop=TRUE]),c(0.75)),
     ...
     line=-0.2,cex.axis=1.5,lwd=0,lwd.ticks=1.5)
box()
text(2.8,0.85,"(b)",cex=1.5)
text(2.2,0.62,"M2",cex=1.5)
text(2.2,0.45,"VEI 3+",cex=1.5)
...
axis(3,at=quantile(log(xx[,1,drop=TRUE]),c(0.995)),
     ...
     line=-0,cex.axis=1.5,lwd=0,lwd.ticks=1.5,col.axis="red")
axis(1,at=quantile(log(xx[,1,drop=TRUE]),c(0.5)),
     floor(quantile((xx[,1,drop=TRUE]),c(0.5))*1000+1500),
     line=-0,cex.axis=1.5,lwd=0,lwd.ticks=1.5)
axis(3,at=quantile(log(xx[,1,drop=TRUE]),c(0.25,0.75)),
...
axis(3,at=quantile(log(xx[,1,drop=TRUE]),c(0.75)),
     ...
     line=-0.2,cex.axis=1.5,lwd=0,lwd.ticks=1.5)
box()
text(3.8,0.46,"(c)",cex=1.5)
text(3.4,0.32,"M3",cex=1.5)
text(3.4,0.25,"VEI 3+",cex=1.5)



#################################
## Forecast x[6] from empirical analogues VEI 3+
...
axis(3,at=quantile(log(xx[,1,drop=TRUE]),c(0.25,0.75)),
     ...
     line=-0.2,cex.axis=1.5,lwd=0,lwd.ticks=1.5)
box()
text(3.8,0.46,"(d)",cex=1.5)
text(3.3,0.34,"M1",cex=1.5)
text(3.3,0.26,"VEI 3+",cex=1.5)
...
axis(3,at=quantile(log(xx[,1,drop=TRUE]),c(0.75)),
     ...
     line=-0.2,cex.axis=1.5,lwd=0,lwd.ticks=1.5)
box()
text(2.2,0.95,"(e)",cex=1.5)
text(1.85,0.7,"M2",cex=1.5)
text(1.85,0.52,"VEI 3+",cex=1.5)
...
axis(3,at=quantile(log(xx[,1,drop=TRUE]),c(0.25))-0.1,
     ...
     line=-0.2,cex.axis=1.5,lwd=0,lwd.ticks=0)
axis(3,at=quantile(log(xx[,1,drop=TRUE]),c(0.75))+0.1,
     floor(quantile((xx[,1,drop=TRUE]),c(0.75))*1000+1500),
     line=-0.2,cex.axis=1.5,lwd=0,lwd.ticks=0)
box()
text(6.9,0.38,"(f)",cex=1.5)
text(5.8,0.28,"M3",cex=1.5)
text(5.8,0.21,"VEI 3+",cex=1.5)



##################################
## Forecast x[14] from long record
...
axis(3,at=quantile(log(xx[,1,drop=TRUE]),c(0.25,0.75)),
     ...
     line=-0.2,cex.axis=1.5,lwd=0,lwd.ticks=1.5)
box()
text(6.7,0.28,"(g)",cex=1.5)
text(5.7,0.23,"M1",cex=1.5)
text(5.7,0.18,"VEI 3+",cex=1.5)
...
axis(3,at=quantile(log(xx[,1,drop=TRUE]),c(0.25,0.75)),
     ...
     line=-0.2,cex.axis=1.5,lwd=0,lwd.ticks=1.5)
box()
text(6.7,0.215,"(h)",cex=1.5)
text(5.7,0.175,"M2",cex=1.5)
text(5.7,0.135,"VEI 3+",cex=1.5)
...
axis(3,at=quantile(log(xx[,1,drop=TRUE]),c(0.25,0.75)),
     ...
     line=-0.2,cex.axis=1.5,lwd=0,lwd.ticks=1.5)
box()
text(6.7,0.265,"(i)",cex=1.5)
text(5.7,0.215,"M3",cex=1.5)
text(5.7,0.165,"VEI 3+",cex=1.5)




###################################
## Forecast x[9] long record VEI 4+
...
axis(3,at=quantile(log(xx[,1,drop=TRUE]),c(0.75)),
     ...
     line=-0.2,cex.axis=1.5,lwd=0,lwd.ticks=1.5)
box()
text(6.2,0.52,"(j)",cex=1.5)
text(5.5,0.42,"M1",cex=1.5)
text(5.5,0.32,"VEI 4+",cex=1.5)
...
axis(3,at=quantile(log(xx[,1,drop=TRUE]),c(0.75)),
     ...
     line=-0.2,cex.axis=1.5,lwd=0,lwd.ticks=1.5)
box()
text(7.2,0.42,"(k)",cex=1.5)
text(6.4,0.34,"M2",cex=1.5)
text(6.4,0.26,"VEI 4+",cex=1.5)
...
axis(3,at=quantile(log(xx[,1,drop=TRUE]),c(0.75)),
     ...
     line=-0.2,cex.axis=1.5,lwd=0,lwd.ticks=1.5)
box()
text(7.2,0.48,"(l)",cex=1.5)
text(6.4,0.4,"M3",cex=1.5)
text(6.4,0.31,"VEI 4+",cex=1.5)
