...
postscript("GVP-EmpV3M1sepZ1.eps")
par(mfrow=c(4,4))
plot(m1.mcmc[,c(1:6,17,22:28)], trace = TRUE, density = FALSE,auto.layout =FALSE)
dev.off()
postscript("GVP-EmpV3M1sepZ2.eps")
par(mfrow=c(4,4))
plot(m1.mcmc[,c(7:16,18:21)], trace = TRUE, density = FALSE,auto.layout =FALSE)
dev.off()
...
postscript("GVP-EmpV3M1postdens.eps",paper="special",
...
apply(cbind(p1c1,p1c2,p1c3),1,mean)
 ...
 [1] 0.04468864 0.00021896 0.47619261 0.00009901 0.01167280 0.09954605
 [7] 0.00016368 0.00472632 0.00003032 0.10143440 0.20092189 0.01407989
[13] 0.21443256 0.15437168 0.38202675 0.04120552 0.00000016 0.01517405
[19] 0.06204437 0.00964648 0.00724875 0.34844880
...
len.chain <- length((m1.Zmcmc[,1])[[1]])
postscript("GVP-EmpV3-M1-Resid-log.eps",paper="special",width=2*5*cos (35.4/180*pi)/0.612,
    height=0.5*3*5,onefile = TRUE, horizontal = FALSE)
par(mfrow=c(5,5),mar=c(4.5,5,1,1))
...
postscript("GVP-EmpV3M2sepZ1.eps")
par(mfrow=c(4,4))
plot(m2.mcmc[,c(1:6,17,22:28)], trace = TRUE, density = FALSE,auto.layout =FALSE)
dev.off()

postscript("GVP-EmpV3M2sepZ2.eps")
par(mfrow=c(4,4))
plot(m2.mcmc[,c(7:16,18:21)], trace = TRUE, density = FALSE,auto.layout =FALSE)
dev.off()
...
postscript("GVP-EmpV3M2postdens.eps",paper="special",
      width=5*cos (35.4/180*pi)/0.612*2,height=5,onefile = TRUE, horizontal = FALSE)
par(mfrow=c(1,2), mar=c(5.1, 5.1, 1, 1))
...
apply(cbind(p2c1,p2c2,p2c3),1,mean)
 ...
 [1] 0.980325 0.906003 0.013534 0.009251 0.889349 0.563136 0.405510 0.517147
 [9] 0.121766 0.189158 0.812441 0.826281 0.619449 0.134108 0.014376 0.419039
[17] 0.374620 0.792028 0.422965 0.928716 0.398154 0.197298
...
len.chain <- length((m2.Zmcmc[,1])[[1]])
postscript("GVP-EmpV3-M2-Resid-log.eps",paper="special",width=2*5*cos (35.4/180*pi)/0.612,
    height=0.5*3*5,onefile = TRUE, horizontal = FALSE)
par(mfrow=c(5,5),mar=c(4.5,5,1,1))
...
floor(quantile((xx[,1,drop=TRUE]),c(0.005,0.25,0.5,0.75,0.995))*1000+1500)
 0.5%   25%   50%   75% 99.5% 
 2024  2485  3298  5069 37695 
...
postscript("GVP-EmpV3M3sepZ-1.eps")
par(mfrow=c(4,4))
plot(m3.mcmc[,c(1:7,18,23:29)], trace = TRUE, density = FALSE,auto.layout =FALSE)
dev.off()
postscript("GVP-EmpV3M3sepZ-2.eps")
par(mfrow=c(4,4))
plot(m3.mcmc[,c(8:17,19:22)], trace = TRUE, density = FALSE,auto.layout =FALSE)
dev.off()

postscript("GVP-EmpV3M3sepZ-1.eps")
par(mfrow=c(4,4))
plot(m3.mcmc[,c(30,41,46:52)], trace = TRUE, density = FALSE,auto.layout =FALSE)
dev.off()
postscript("GVP-EmpV3M3sepZ-2.eps")
par(mfrow=c(4,4))
plot(m3.mcmc[,c(31:40,42:45)], trace = TRUE, density = FALSE,auto.layout =FALSE)
dev.off()
...
postscript("GVP-EmpV3M3postdens.eps",paper="special",
...
len.chain <- length((m3.Zmcmc[,1])[[1]])
postscript("GVP-EmpV3-M3-Resid-log.eps",paper="special",width=2*5*cos (35.4/180*pi)/0.612,
    height=0.5*3*5,onefile = TRUE, horizontal = FALSE)
par(mfrow=c(5,5),mar=c(4.5,5,1,1))

a2h <- c("(a)","(b)","(c)","(d)","(e)","(f)","(g)","(h)","(i)","(j)","(k)",
...
options("scipen"=100, "digits"=4)
t(t(apply(cbind(p1c1,p1c2,p1c3),1,mean)))
t(t(apply(cbind(p2c1,p2c2,p2c3),1,mean)))
t(t(apply(cbind(p3c1,p3c2,p3c3),1,mean)))
t(t(apply(cbind(pY3c1,pY3c2,pY3c3),1,mean)))
...
## Forecast x[6]
postscript("GVPV3-Emprec-M123-Forecast.eps",paper="special",
