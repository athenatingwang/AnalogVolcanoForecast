...
xi[9]     13.002 21.488    2.568    4.034 1.00 149287
MCMCsummary(m1.mcmc,HPD=TRUE,hpd_prob=0.5,round=3)
            mean     sd 50%_HPDL 50%_HPDU Rhat  n.eff
xi[9]     13.002 21.488    2.568    6.820 1.00 149287
MCMCsummary(m1.mcmc,HPD=TRUE,hpd_prob=0.95,round=3)
            mean     sd 95%_HPDL 95%_HPDU Rhat  n.eff
xi[9]     13.002 21.488    2.568   42.008 1.00 149287

forex.m1 <- 1000*scan()
2.568    4.034 2.568    6.820  2.568   42.008

forex.m1-550
[1]  2018  3484  2018  6270  2018 41458


# Model 2
MCMCsummary(m2.mcmc,HPD=TRUE,hpd_prob=0.25,round=3)
            mean     sd 95%_HPDL 95%_HPDU Rhat  n.eff
xi[9]     19.981 66.329    2.568    4.490 1.00 140733
MCMCsummary(m2.mcmc,HPD=TRUE,hpd_prob=0.5,round=3)
            mean     sd 95%_HPDL 95%_HPDU Rhat  n.eff
xi[9]     19.981 66.329    2.568    8.280 1.00 140733
MCMCsummary(m2.mcmc,HPD=TRUE,hpd_prob=0.95,round=3)
            mean     sd 50%_HPDL 50%_HPDU Rhat  n.eff
xi[9]     19.981 66.329    2.568   65.917 1.00 140733

forex.m2 <- 1000*scan()
2.568    4.490 2.568    8.280  2.568   65.917

forex.m2-550
[1]  2018  3940  2018  7730  2018 65367


# Model 3
MCMCsummary(m3.mcmc,HPD=TRUE,hpd_prob=0.25,round=3)
            mean     sd 95%_HPDL 95%_HPDU Rhat  n.eff
xi[9]     26.434 625.302    2.569    4.189 1.13 150000
MCMCsummary(m3.mcmc,HPD=TRUE,hpd_prob=0.5,round=3)
            mean     sd 95%_HPDL 95%_HPDU Rhat  n.eff
xi[9]     26.434 625.302    2.568    7.423 1.13 150000
MCMCsummary(m3.mcmc,HPD=TRUE,hpd_prob=0.95,round=3)
            mean     sd 50%_HPDL 50%_HPDU Rhat  n.eff
xi[9]     26.434 625.302    2.568   63.279 1.13 150000

forex.m3 <- 1000*scan()
2.569    4.189  2.568    7.423  2.568   63.279

forex.m3-550
[1]  2019  3639  2018  6873  2018 62729
...
#quantile((xx[,1,drop=TRUE]),c(0.025,0.25,0.5,0.75,0.975))*1000-550
# 2134.067  3484.361  6269.855 13093.220 60863.224 
...
abline(v=quantile(log(xx[,1,drop=TRUE]),c(0.025,0.25,0.5,0.75,0.975)),
     lty="dashed",lwd=2,col="blue")
#quantile((xx[,1,drop=TRUE]),c(0.025,0.25,0.5,0.75,0.975))*1000-550
#  2160.362   3939.914   7729.033  17690.098 103160.516
axis(1,at=quantile(log(xx[,1,drop=TRUE]),c(0.025,0.5,0.975)),
     c(2160,7729,103160),
     line=-0,cex.axis=1.4,lwd=0,lwd.ticks=1.5)
axis(1,at=quantile(log(xx[,1,drop=TRUE]),c(0.5)),
...
abline(v=quantile(log(xx[,1,drop=TRUE]),c(0.025,0.25,0.5,0.75,0.975)),
     lty="dashed",lwd=2,col="blue")
# quantile((xx[,1,drop=TRUE]),c(0.025,0.25,0.5,0.75,0.975))*1000-550
#  2142.487   3638.315   6872.997  15553.067 102884.552 
axis(1,at=quantile(log(xx[,1,drop=TRUE]),c(0.025,0.5,0.975)),
     c(2142,6872,102884),
     line=-0,cex.axis=1.4,lwd=0,lwd.ticks=1.5)
axis(1,at=quantile(log(xx[,1,drop=TRUE]),c(0.5)),
